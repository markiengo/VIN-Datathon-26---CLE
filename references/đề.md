...
> Ghi chú đồng bộ repo:
> - Bản PDF gốc hiện có trong repo là `references/đề_og_btc.pdf`.
> - PDF vẫn dùng cách gọi `sales_train.csv` / `sales_test.csv` ở một số đoạn; trong repo phát hành, file train public là `dataset/sales.csv`, còn horizon test public được biểu diễn qua `dataset/sample_submission.csv` với 548 dòng từ `2023-01-01` đến `2024-07-01`.
> - Câu Q1 đã được cập nhật theo PDF mới: đáp án C là `144 ngày`.
...
1. [Mô tả Dữ liệu](#mô-tả-dữ-liệu)
   ...
   2. [Tổng quan các bảng dữ liệu](#tổng-quan-các-bảng-dữ-liệu)
   3. [Bảng Master](#bảng-master)
      1. [products.csv — Danh mục sản phẩm](#productscsv--danh-mục-sản-phẩm)
      2. [customers.csv — Khách hàng](#customerscsv--khách-hàng)
      3. [promotions.csv — Chương trình khuyến mãi](#promotionscsv--chương-trình-khuyến-mãi)
      4. [geography.csv — Địa lý](#geographycsv--địa-lý)
   4. [Bảng Transaction](#bảng-transaction)
      1. [orders.csv — Đơn hàng](#orderscsv--đơn-hàng)
      2. [order_items.csv — Chi tiết đơn hàng](#order_itemscsv--chi-tiết-đơn-hàng)
      3. [payments.csv — Thanh toán](#paymentscsv--thanh-toán)
      4. [shipments.csv — Vận chuyển](#shipmentscsv--vận-chuyển)
      5. [returns.csv — Trả hàng](#returnscsv--trả-hàng)
      6. [reviews.csv — Đánh giá](#reviewscsv--đánh-giá)
   5. [Bảng Analytical](#bảng-analytical)
      1. [sales.csv — Dữ liệu doanh thu](#salescsv--dữ-liệu-doanh-thu)
   6. [Bảng Operational](#bảng-operational)
      1. [inventory.csv — Tồn kho](#inventorycsv--tồn-kho)
      2. [web_traffic.csv — Lưu lượng truy cập](#web_trafficcsv--lưu-lượng-truy-cập)
   7. [Quan hệ giữa các bảng](#quan-hệ-giữa-các-bảng)
2. [Đề Bài](#đề-bài)
   1. [Phần 1 — Câu hỏi Trắc nghiệm](#phần-1--câu-hỏi-trắc-nghiệm)
   2. [Phần 2 — Trực quan hoá và Phân tích Dữ liệu](#phần-2--trực-quan-hoá-và-phân-tích-dữ-liệu)
      1. [Yêu cầu](#yêu-cầu)
   3. [Phần 3 — Mô hình Dự báo Doanh thu (Sales Forecasting)](#phần-3--mô-hình-dự-báo-doanh-thu-sales-forecasting)
      1. [Bối cảnh kinh doanh](#bối-cảnh-kinh-doanh)
      2. [Định nghĩa bài toán](#định-nghĩa-bài-toán)
      3. [Dữ liệu](#dữ-liệu)
      4. [Chỉ số đánh giá](#chỉ-số-đánh-giá)
      5. [Định dạng file nộp](#định-dạng-file-nộp)
      6. [Ràng buộc](#ràng-buộc)
3. [Thang điểm Chấm thi](#thang-điểm-chấm-thi)
4. [Hướng dẫn Nộp bài](#hướng-dẫn-nộp-bài)
   1. [Checklist nộp bài](#checklist-nộp-bài)
...
Bộ dữ liệu mô phỏng hoạt động của một doanh nghiệp thời trang thương mại điện tử tại Việt Nam trong giai đoạn từ 04/07/2012 đến 31/12/2022. Bản phát hành trong repo gồm 14 file CSV, được chia thành 4 lớp: Master (dữ liệu tham chiếu), Transaction (giao dịch), Analytical (phân tích) và Operational (vận hành).

Phân chia dữ liệu cho bài toán dự báo:

- **sales.csv:** 04/07/2012 → 31/12/2022
- **sample_submission.csv:** 01/01/2023 → 01/07/2024 (template public cho horizon test ẩn mà đề vẫn gọi là `sales_test.csv`)
...
| Test | sample_submission.csv | 01/01/2023 – 01/07/2024 |
...
**Q7.** Vùng (region) nào trong geography.csv tạo ra tổng doanh thu cao nhất trong sales.csv (train split)?
...
Dự báo cột `Revenue` trong khoảng thời gian của horizon test ẩn, với template public là `sample_submission.csv`.
...
`MAE = (1/n) Σ(i=1→n) |Fi − Ai|`

**Root Mean Squared Error (RMSE):**

`RMSE = √[(1/n) Σ(i=1→n) (Fi − Ai)^2]`

**R² (Coefficient of Determination):**

`R² = 1 − [Σ(i=1→n)(Ai − Fi)^2 / Σ(i=1→n)(Ai − Ā)^2]`

trong đó `Fi` là giá trị dự báo, `Ai` là giá trị thực, `Ā` là trung bình giá trị thực. MAE đo độ lệch tuyệt đối trung bình, RMSE phạt nặng hơn các sai số lớn, và `R²` thể hiện tỷ lệ phương sai được giải thích bởi mô hình.

MAE và RMSE càng thấp càng tốt. R² càng cao càng tốt (lý tưởng gần 1).
...
|---|---|---|
| Hiệu suất mô hình | Dựa trên điểm MAE, RMSE, R2 trên tập test (Kaggle leaderboard); xếp hạng tương đối so với các đội khác trong cuộc thi | 12 |
...
|---|---|---|
| Hiệu suất mô hình (12đ) | 10–12đ | Xếp hạng top leaderboard; MAE và RMSE thấp, R2 cao |
